...
//2017B3A70373H   Pranav Sai Itapu
//2017B3A70381H   Vashist SLN
#include<bits/stdc++.h>
using namespace std;
...
vector<string> keyWords= {"int", "float", "boolean", "string", "while", "letter", "if", "else", "true", "false","printf","endl","return","until","input","output","null","main"};
    ...
    while(tok[i] != '\0' && state != 10)
    {
        ...
        switch(state)
        {
            ...
            case 9:
                ...
                break;
            case 10:
                cout<<"invalid tokens are present"<<endl;
                return 10;
            case 11:
                if(t == '.')
                    state = 7;
...
int tokenizeBuffer(string tok)
{
    ...
    {   
        ...
        {
            //Boolean op.
            if(tok[i] == '=' || tok[i] == '&' || tok[i] == '|' || tok[i]=='<' || tok[i]=='>')
            {
                buffer+=tok[i];
                flag1 = 2;
            }
